...
const build_accessibility_report_options_1 = require("../ufg/build-accessibility-report-options");
...
async function runOfflineSnapshots(options) {
    ...
    // figmaBaselines/figmaOptions are off-the-wire open config that the open seam resolves and then strips.
    // This replay path bypasses the seam, so strip them once here — figmaOptions carries a Figma access
    // token, and getAccountInfo logs the settings it is called with.
    const openConfig = { ...options.config.open };
    delete openConfig.figmaBaselines;
    delete openConfig.figmaOptions;
    const eyesServerSettings = { ...openConfig, ...options };
    ...
    async function runTests(testFolders) {
        ...
        const testPromises = testFolders.map(async (testFolder) => {
            ...
            const fileOpenSettings = await fs_1.default.promises
                ...
                .then(JSON.parse);
            const openSettings = (0, merge_configs_1.mergeConfigs)(fileOpenSettings, openConfig);
    ...
    async function render(target, logger) {
        ...
        const accessibilityReportOptions = target.settings.accessibilitySettings
            ? (0, build_accessibility_report_options_1.buildAccessibilityReportOptions)({ accessibilityOptions: account.accessibilityOptions })
            : undefined;
        ...
        if (accessibilityReportOptions === null || accessibilityReportOptions === void 0 ? void 0 : accessibilityReportOptions.uploadUrl)
            (_b = baseTarget.accessibilityReportLocation) !== null && _b !== void 0 ? _b : (baseTarget.accessibilityReportLocation = accessibilityReportOptions.uploadUrl);
