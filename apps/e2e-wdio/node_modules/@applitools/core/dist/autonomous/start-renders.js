...
function makeStartRenders({ core, logger: defaultLogger }) {
    return async function startRenders({ targets, logger = defaultLogger, }) {
        ...
        const renders = await Promise.all(Array.from(clients.entries(), async ([ufgClient, targets]) => {
            const accessibilityReportOptions = targets.map(target => target.settings.accessibilitySettings
                ? (0, build_accessibility_report_options_1.buildAccessibilityReportOptions)({ accessibilityOptions: target.account.accessibilityOptions })
                : undefined);
            const [renders, ...environments] = await Promise.all([
                ufgClient.startRenders({
                    renders: targets.map((target, index) => {
                        var _a;
                        ...
                        return {
                            ...
                            settings: {
                                ...
                                accessibilityReportOptions: accessibilityReportOptions[index],
                            },
                        };
                    }),
                    logger,
            ...
            return renders.map((render, index) => {
                ...
                return ({
                    ...
                    accessibilityReportLocation: (_a = accessibilityReportOptions[index]) === null || _a === void 0 ? void 0 : _a.uploadUrl,
                });
            });
        }));
        return renders.flat();
