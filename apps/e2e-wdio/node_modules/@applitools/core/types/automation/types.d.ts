...
import type { FigmaOptions } from '../figma/types';
...
export type OpenSettings = BaseCore.OpenSettings & {
    ...
    bypassCSP?: boolean;
    /**
     * @internal Resolved in core's open-eyes (the Figma seam) before session creation and stripped
     * from the wire config — it is not part of the cross-language wire contract, but is available to
     * every SDK that routes its opens through core.
     */
    figmaBaselines?: string | Record<string, string | string[]>;
    /** @internal Read and stripped by the Figma seam during open, like `figmaBaselines` — never reaches the wire. */
