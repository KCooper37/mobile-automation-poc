...
const server_concurrency_converter_1 = require("../utils/server-concurrency-converter");
const addRetryTimeout_1 = require("../utils/addRetryTimeout");
...
function makeCoreRequests({ fetch, logger: defaultLogger } = {}) {
    ...
    const getAccountInfoWithCache = utils.general.cachify(getAccountInfo, ([{ settings }]) => {
        return [settings.eyesServerUrl, settings.apiKey, settings.proxy, settings.httpVersion];
    });
    ...
    async function openCheckAndClose({ target, settings, logger = mainLogger, }) {
        ...
        const openCheckAndCloseResponse = req('./api/sessions/autonomous-start-match-end', {
            ...
            method: 'POST',
            ...(settings.httpVersion ? { httpVersion: settings.httpVersion } : {}),
            body: {
                startInfo: {
                    agentId: settings.agentId,
                    agentSessionId: settings.userTestId,
    ...
    async function getAccountInfo({ settings, logger = mainLogger, }) {
        ...
        const { serviceUrl: ufgServerUrl, serviceUrlNew: ufgServerUrlNew, accessToken, mobileDevicesListUrl: supportedEnvironmentsUrl, resultsUrl: uploadUrl, orgSessionConcurrencyLimits: serverConcurrency, ...rest } = res;
        const result = {
            eyesServer: {
                eyesServerUrl: settings.eyesServerUrl,
                apiKey: settings.apiKey,
            ...
            hooks: {
                afterError({ error }) {
                    if (error.message.includes('Bad Request(400)') ||
                        error.message.includes('Not Found(404)') || // validation issue in start session
...
function makeEyesRequests({ core, test, req: defaultReq, fetch, logger: mainLogger, }) {
    ...
    const upload = (0, upload_1.makeUpload)({
        settings: {
            ...
            proxy: test.eyesServer.proxy,
            httpVersion: test.eyesServer.httpVersion,
        },
        logger: mainLogger,
    });
    const eyes = {
    ...
    async function check({ target, settings, logger = mainLogger, }) {
        ...
        const httpVersion = (_a = settings.httpVersion) !== null && _a !== void 0 ? _a : test.eyesServer.httpVersion;
...
function toServerMatchOptions({ target, settings }) {
    ...
    return {
        appOutput: {
            ...
            screenshotUrl: target.image,
            domUrl: target.dom,
            accessibilityReportUrl: target.accessibilityReportLocation,
