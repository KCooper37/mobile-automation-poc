...
const IosMultiDeviceTarget_1 = require("../enums/IosMultiDeviceTarget");
const AndroidMultiDeviceTarget_1 = require("../enums/AndroidMultiDeviceTarget");
const IosDeviceTarget_1 = require("./IosDeviceTarget");
const AndroidDeviceTarget_1 = require("./AndroidDeviceTarget");
...
class ConfigurationData {
    ...
    addMultiDeviceTarget(...devices) {
        ...
        for (const device of devices) {
            // Check if it's a Target class instance (has getDeviceName method)
            if (device instanceof IosDeviceTarget_1.IosDeviceTarget || device instanceof AndroidDeviceTarget_1.AndroidDeviceTarget) {
                const deviceName = device.getDeviceName();
                const screenOrientation = device.getOrientation();
                // Auto-detect platform
                if (device instanceof IosDeviceTarget_1.IosDeviceTarget || this._isIosDevice(deviceName)) {
                ...
                // Auto-detect platform by checking which const object the device belongs to
                if (this._isIosDevice(deviceName)) {
                    this.browsersInfo.push({ iosDeviceInfo: { deviceName: deviceName } });
                }
                else if (this._isAndroidDevice(deviceName)) {
                    this.browsersInfo.push({ androidDeviceInfo: { deviceName: deviceName } });
                }
                else {
                    throw new Error(`Invalid device name: "${deviceName}". ` +
                        `Supported iOS devices: ${Object.values(IosMultiDeviceTarget_1.IosMultiDeviceTarget)
                            .map(d => d.getDeviceName())
                            .join(', ')}. ` +
                        `Supported Android devices: ${Object.values(AndroidMultiDeviceTarget_1.AndroidMultiDeviceTarget)
                            .map(d => d.getDeviceName())
                            .join(', ')}.`);
                }
            }
    ...
    _isIosDevice(deviceName) {
        return Object.values(IosMultiDeviceTarget_1.IosMultiDeviceTarget)
            .map(target => target.getDeviceName())
            .includes(deviceName);
    }
    _isAndroidDevice(deviceName) {
        return Object.values(AndroidMultiDeviceTarget_1.AndroidMultiDeviceTarget)
            .map(target => target.getDeviceName())
            .includes(deviceName);
    }
    ...
    get figmaBaselines() {
        return this._config.figmaBaselines;
    }
    set figmaBaselines(figmaBaselines) {
        if (!utils.types.isString(figmaBaselines)) {
            utils.guard.isObject(figmaBaselines, { name: 'figmaBaselines' });
        }
        this._config.figmaBaselines = figmaBaselines;
    }
    getFigmaBaselines() {
        return this.figmaBaselines;
    }
    setFigmaBaselines(figmaBaselines) {
        this.figmaBaselines = figmaBaselines;
        return this;
    }
    get figmaOptions() {
        return this._config.figmaOptions;
    }
    set figmaOptions(figmaOptions) {
        utils.guard.isObject(figmaOptions, { name: 'figmaOptions' });
        this._config.figmaOptions = figmaOptions;
    }
    getFigmaOptions() {
        return this.figmaOptions;
    }
    setFigmaOptions(figmaOptions) {
        this.figmaOptions = figmaOptions;
        return this;
    }
    get useDom() {
    ...
    toJSON() {
        ...
        // Re-validate `exact` at the conversion site: the `defaultMatchSettings` setter validates at set-time,
        // but the user may keep a reference to the tuning object and mutate it afterwards. Gated on
        // matchLevel === Exact to mirror forwarding semantics — core-base drops `exact` under any other
        // matchLevel, so a stray invalid tuning there stays ignored rather than throwing.
        ...
        return utils.general.toJSON({
            open: utils.general.removeUndefinedProps({
                ...
                // Emit only when set — an always-present `figmaBaselines: undefined` would change the
                // serialized open-config shape and break offline snapshot comparisons.
                ...(this.figmaBaselines !== undefined ? { figmaBaselines: this.figmaBaselines } : {}),
                ...(this.figmaOptions !== undefined ? { figmaOptions: this.figmaOptions } : {}),
            ...
            screenshot: utils.general.removeUndefinedProps({
                ...
                debugImages: ((_c = this.debugScreenshots) === null || _c === void 0 ? void 0 : _c.save) && utils.types.has(this.debugScreenshots, 'path')
            ...
            check: utils.general.removeUndefinedProps({
                ...
                layoutBreakpoints: utils.types.isDefined(this.layoutBreakpoints)
                    ? utils.types.has(this.layoutBreakpoints, 'breakpoints')
                        ...
                        : { breakpoints: (_e = this.layoutBreakpoints) !== null && _e !== void 0 ? _e : false }
                    : undefined,
