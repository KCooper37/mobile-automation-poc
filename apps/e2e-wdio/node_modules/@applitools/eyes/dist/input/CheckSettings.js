...
class CheckSettingsImageFluent extends CheckSettingsBaseFluent {
    ...
    toJSON() {
        ...
        // Validate `exact` on the plain-object path too (e.g. `eyes.check({matchLevel: 'Exact', exact: {...}})`),
        // which bypasses the fluent `.exact()` setter. Gated on matchLevel === Exact to mirror forwarding
        // semantics — core-base only forwards `exact` when matchLevel is Exact and drops it otherwise, so a
        // stray invalid tuning under any other matchLevel stays ignored rather than throwing.
